...
public:
  unordered_map<CInvoice,bool,CInvoicehash<CInvoice>,CInvoiceEqual<CInvoice>> m_issue;
  unordered_map<CInvoice,bool,CInvoicehash<CInvoice>,CInvoiceEqual<CInvoice>> m_accept;
  Ccompany() {};
  Ccompany(const string &);
...
{ 
  ...
  companies[cannonnical_seller].m_issue[y] = true;
    cout << "In the table: " << endl;
    auto element = companies.find(cannonnical_buyer);
  return true;
}
...
{ 
  ...
  if(companies.find(cannonnical_seller)->second.m_accept[y] == true)
  {
    return false;
  }

  if(companies.find(cannonnical_buyer)->second.m_accept[y] ==  true )
  {
    return false;
  }
...
bool CVATRegister::delIssued(const CInvoice &x)
{
  ...
  if(companies.find(cannonnical_seller)->second.m_issue[y] == false )
  {
    return false;
  }
  ...
  m_order++;
  y.changeorder(m_order);
  companies.find(cannonnical_buyer)->second.m_issue[y] = false;
  companies.find(cannonnical_seller)->second.m_issue[y] = false;
  return true;
}
...
bool CVATRegister::delAccepted(const CInvoice &x)
{
  ...
  if(companies.find(cannonnical_seller)->second.m_accept[y] == false )
  {
    return false;
  }
  ...
  companies.find(cannonnical_buyer)->second.m_accept[y] = false;
  companies.find(cannonnical_seller)->second.m_accept[y] = false;
  return true;
}
...
list<CInvoice> CVATRegister::unmatched(const string &company,const CSortOpt &sortBy) const
{
  ...
  if(iterator != companies.end())
  {
    ...
    {
      if(iterator->second.m_accept.find(invoice.first) == iterator->second.m_accept.end() || iterator->second.m_accept.find(invoice.first)->second == false)
