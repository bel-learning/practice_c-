...
namespace my
{
  ...
  class string {
    ...
    friend ostream & operator << (ostream & out, const  string & me) {
        out << me._data;
        return out;
    }
  ...
  class svector {
    ...
    public:
        ...
        SNode(const my::string & str): val(str), left(nullptr), right(nullptr) {}; 
    };
    ...
    public:
        ...
        SNode * insert(const my::string & val) {
        ...
        SNode * search(const my::string & val) {
            SNode* cur = root;
            while (cur != NULL) {
                if (val == cur->val) {
                    return cur;
    ...
    class BST_SHARED {
        ...
        BST_SHARED(): m_Ptr(new SBST) {};
        BST_SHARED(const BST_SHARED & s) {
            m_Ptr = s.m_Ptr;
            m_Ptr->increaseRef();
        }
        BST_SHARED & operator = (const BST_SHARED & s) {
        ...
        SNode * stringToNode(const my::string & val) {
            SNode * searchFound = m_Ptr->search(val);
            if(searchFound == nullptr) {
                return m_Ptr->insert(val);
            }
            else {
                return searchFound;
            }
...
public:
  ...
  CMail(const char *from,
        ...
        const char *body)
      {
        _from = my::string(from);
        _to = my::string(to);
        _body = my::string(body);
      };
